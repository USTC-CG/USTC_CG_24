...
#include "GenShaderUtil.h"
#include "gtest/gtest.h"

namespace mx = MaterialX;

TEST(GenShader, GLSL_Syntax_Check)
{
...
TEST(GenShader, Implementation_Check)
{
    mx::GenContext context(mx::GlslShaderGenerator::create());

    mx::StringSet generatorSkipNodeTypes;
    mx::StringSet generatorSkipNodeDefs;
    GenShaderUtil::checkImplementations(
        context, generatorSkipNodeTypes, generatorSkipNodeDefs, 47);
}

TEST(GenShader, Unique_Names)
{
    mx::GenContext context(mx::GlslShaderGenerator::create());
...
TEST(GenShader, Performance Test)
{
    mx::GenContext context(mx::GlslShaderGenerator::create());
...
static void generateGlslCode(GlslType type = GlslType::Glsl400)
{
    ...
    GlslShaderGeneratorTester tester(
...
TEST(GenShader, Shader_Generation)
{
    // Generate with standard  i.e version 400
    generateGlslCode(GlslType::Glsl400);
}

TEST(GenShader, Shader_with_Layout_Generation)
{
    // Generate  with layout i.e version 400 + layout extension
    generateGlslCode(GlslType::Glsl420);
}

TEST(GenShader, Vulkan_Shader)
{
    // Generate with  for Vulkan i.e. version 450
