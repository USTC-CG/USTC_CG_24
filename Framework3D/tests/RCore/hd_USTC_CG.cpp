...
#include "RCore/hd_USTC_CG/renderDelegate.h"
#include "RCore/hd_USTC_CG/rendererPlugin.h"
#include "pxr/pxr.h"
...
class hd_USTC_CG_UnitestDelegate : public HdUnitTestDelegate
{
public:
    hd_USTC_CG_UnitestDelegate(
...
private:
    ...
    HdEngine _engine;
    Hd_USTC_CG_RendererPlugin* _rendererPlugin;
    HdRenderDelegate* _renderDelegate;
    HdRenderIndex* _renderIndex;
    hd_USTC_CG_UnitestDelegate* _sceneDelegate;
...
void HdEmbree_TestGLDrawing::InitTest()
{
    ...
    // a render index.
    _rendererPlugin = new Hd_USTC_CG_RendererPlugin;
    ...
    _sceneDelegate =
        new hd_USTC_CG_UnitestDelegate(_renderIndex, SdfPath::AbsoluteRootPath());
    ...
    if (_aov.size() > 0)
    {
        ...
        _sceneDelegate->AddRenderBuffer(
            ...
            HdRenderBufferDescriptor{
                GfVec3i(GetWidth(), GetHeight(), 1), format, false });
    }
    ...
    if (_instance)
    {
        ...
        _sceneDelegate->SetInstancerProperties(
            SdfPath("/instancer"),
            index,
            scale,
            rotate,
    ...
    _sceneDelegate->GetRenderIndex().InsertSprim(
        HdPrimTypeTokens->simpleLight,
        _sceneDelegate,
        SdfPath("/pointLight"));

    GfMatrix4f light;
    light.SetTranslateOnly(GfVec3f(5, 0, 5));

    _sceneDelegate->UpdateTransform(SdfPath("/pointLight"), light);
...
void HdEmbree_TestGLDrawing::OffscreenTest()
{
    ...
    if (_aov.size() > 0)
    {
        ...
        auto rb =
            static_cast<HdRenderBuffer*>(_renderIndex->GetBprim(
...
void HdEmbree_TestGLDrawing::ParseArgs(int argc, char* argv[])
{
    ...
    if (_aov.size() > 0 && _aov != "color" && _aov != "cameraDepth" &&
        _aov != "primId")
    {
