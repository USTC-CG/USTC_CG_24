...
namespace ax
{
    ...
    {
        //------------------------------------------------------------------------------
        struct USTC_CG::NodeId;
        struct USTC_CG::LinkId;
        struct USTC_CG::SocketID;
        ...
        inline SaveReasonFlags operator |(
            ...
            SaveReasonFlags rhs)
        {
            return static_cast<SaveReasonFlags>(
        ...
        using ConfigSaveNodeSettings = bool (*)(
            ...
            SaveReasonFlags reason,
            void* userPointer);
        using ConfigLoadNodeSettings = size_t (*)(
            NodeId nodeId,
            char* data,
            void* userPointer);
        ...
        struct Config
        {
            ...
            int DragButtonIndex;
            // Mouse button index drag action will react to (0-left, 1-right, 2-middle)
            int SelectButtonIndex;
            // Mouse button index select action will react to (0-left, 1-right, 2-middle)
            int NavigateButtonIndex;
            // Mouse button index navigate action will react to (0-left, 1-right, 2-middle)
            int ContextMenuButtonIndex;
            // Mouse button index context menu action will react to (0-left, 1-right, 2-middle)
...
# endif
        ...
        struct Style
        {
            ...
            // when true link will start on the line defined by pin direction
        ...
        PinCorners               = ImDrawCornerFlags_All;
                ...
                Colors[StyleColor_NodeSelRectBorder] = ImColor(
                    5,
                    130,
                    255,
                    128);
                ...
                Colors[StyleColor_HighlightLinkBorder] = ImColor(
                    204,
                    105,
                    0,
                    255);
                Colors[StyleColor_LinkSelRect] = ImColor(5, 130, 255, 64);
                Colors[StyleColor_LinkSelRectBorder] = ImColor(
                    5,
                    130,
                    255,
                    128);
        ...
        IMGUI_NODE_EDITOR_API EditorContext* CreateEditor(
            const Config* config = nullptr);
        IMGUI_NODE_EDITOR_API void DestroyEditor(EditorContext* ctx);
        IMGUI_NODE_EDITOR_API const Config& GetConfig(
            EditorContext* ctx = nullptr);

        IMGUI_NODE_EDITOR_API Style& GetStyle();
        IMGUI_NODE_EDITOR_API const char* GetStyleColorName(
            StyleColor colorIndex);

        IMGUI_NODE_EDITOR_API void PushStyleColor(
            StyleColor colorIndex,
            const ImVec4& color);
        ...
        IMGUI_NODE_EDITOR_API void PushStyleVar(
            StyleVar varIndex,
            const ImVec2& value);
        IMGUI_NODE_EDITOR_API void PushStyleVar(
            StyleVar varIndex,
        ...
        IMGUI_NODE_EDITOR_API bool Link(
            LinkId id,
            SocketID startPinId,
            SocketID endPinId,
        ...
        IMGUI_NODE_EDITOR_API void Flow(
            LinkId linkId,
            FlowDirection direction = FlowDirection::Forward);

        IMGUI_NODE_EDITOR_API bool BeginCreate(
            const ImVec4& color = ImVec4(1, 1, 1, 1),
            float thickness = 1.0f);
        IMGUI_NODE_EDITOR_API bool QueryNewLink(SocketID* startId, SocketID* endId);
        IMGUI_NODE_EDITOR_API bool QueryNewLink(
            SocketID* startId,
            SocketID* endId,
            const ImVec4& color,
            float thickness = 1.0f);
        IMGUI_NODE_EDITOR_API bool QueryNewNode(SocketID* pinId);
        IMGUI_NODE_EDITOR_API bool QueryNewNode(
            SocketID* pinId,
            const ImVec4& color,
            float thickness = 1.0f);
        ...
        IMGUI_NODE_EDITOR_API void SetNodeZPosition(
            NodeId nodeId,
            float z);
        // Sets node z position, nodes with higher value are drawn over nodes with lower value
        IMGUI_NODE_EDITOR_API float GetNodeZPosition(NodeId nodeId);
        ...
        IMGUI_NODE_EDITOR_API int GetSelectedNodes(
            NodeId* nodes,
            int size);
        ...
        IMGUI_NODE_EDITOR_API void SelectNode(
            NodeId nodeId,
            bool append = false);
        IMGUI_NODE_EDITOR_API void SelectLink(
            LinkId linkId,
            bool append = false);
        ...
        IMGUI_NODE_EDITOR_API bool HasAnyLinks(NodeId nodeId);
        // Returns true if node has any link connected
        IMGUI_NODE_EDITOR_API bool HasAnyLinks(SocketID pinId);
        // Return true if pin has any link connected
        IMGUI_NODE_EDITOR_API int BreakLinks(NodeId nodeId);
        // Break all links connected to this node
        IMGUI_NODE_EDITOR_API int BreakLinks(SocketID pinId);
        // Break all links connected to this pin

        IMGUI_NODE_EDITOR_API void NavigateToContent(float duration = -1);
        IMGUI_NODE_EDITOR_API void NavigateToSelection(
            bool zoomIn = false,
            float duration = -1);

        IMGUI_NODE_EDITOR_API bool ShowNodeContextMenu(NodeId* nodeId);
        IMGUI_NODE_EDITOR_API bool ShowPinContextMenu(SocketID* pinId);
        ...
        IMGUI_NODE_EDITOR_API int GetActionContextNodes(
            NodeId* nodes,
            int size);
        IMGUI_NODE_EDITOR_API int GetActionContextLinks(
            LinkId* links,
            int size);
        ...
        IMGUI_NODE_EDITOR_API SocketID GetHoveredPin();
        IMGUI_NODE_EDITOR_API LinkId GetHoveredLink();
        IMGUI_NODE_EDITOR_API NodeId GetDoubleClickedNode();
        IMGUI_NODE_EDITOR_API SocketID GetDoubleClickedPin();
        ...
        IMGUI_NODE_EDITOR_API ImGuiMouseButton GetBackgroundClickButtonIndex();
        // -1 if none
        IMGUI_NODE_EDITOR_API ImGuiMouseButton
        GetBackgroundDoubleClickButtonIndex(); // -1 if none

        IMGUI_NODE_EDITOR_API bool GetLinkPins(
            LinkId linkId,
            SocketID* startPinId,
            SocketID* endPinId);
        // pass nullptr if particular pin do not interest you

        IMGUI_NODE_EDITOR_API bool PinHadAnyLinks(SocketID pinId);
        ...
        IMGUI_NODE_EDITOR_API int GetNodeCount();
        // Returns number of submitted nodes since Begin() call
        IMGUI_NODE_EDITOR_API int GetOrderedNodeIds(
            NodeId* nodes,
            int size);
