...
/// \class Hd_USTC_CG_HWRT_Instancer
///
/// Hd_USTC_CG_HWRT_ implements instancing by adding prototype geometry to the BVH
/// multiple times within Hd_USTC_CG_HWRT_Mesh::Sync(). The only instance-varying
/// attribute that Hd_USTC_CG_HWRT_ supports is transform, so the natural
...
///
class Hd_USTC_CG_HWRT_Instancer : public HdInstancer
{
public:
    /// Constructor.
    ///   \param delegate The scene delegate backing this instancer's data.
    ///   \param id The unique id of this instancer.
    Hd_USTC_CG_HWRT_Instancer(HdSceneDelegate* delegate, SdfPath const& id);

    /// Destructor.
    ~Hd_USTC_CG_HWRT_Instancer();
