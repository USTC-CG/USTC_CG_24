...
void Hd_USTC_CG_GL_Renderer_USTC_CG_GL::Render(HdRenderThread* renderThread)
{
    ...
    if (!_ValidateAovBindings()) {
        ...
        for (size_t i = 0; i < _aovBindings.size(); ++i) {
            auto rb = static_cast<Hd_USTC_CG_GL_RenderBuffer*>(_aovBindings[i].renderBuffer);
...
void Hd_USTC_CG_GL_Renderer_USTC_CG_GL::Clear()
{
    if (!_ValidateAovBindings()) {
        return;
    }
...
/* static */
GfVec4f Hd_USTC_CG_GL_Renderer_USTC_CG_GL::_GetClearColor(const VtValue& clearValue)
{
