...
const TfTokenVector Hd_USTC_CG_GL_RenderDelegate::SUPPORTED_SPRIM_TYPES = {
    HdPrimTypeTokens->camera,
    HdPrimTypeTokens->simpleLight,
    HdPrimTypeTokens->sphereLight,
};

const TfTokenVector Hd_USTC_CG_GL_RenderDelegate::SUPPORTED_BPRIM_TYPES = {
    HdPrimTypeTokens->renderBuffer,
};

Hd_USTC_CG_GL_RenderDelegate::Hd_USTC_CG_GL_RenderDelegate() : HdRenderDelegate()
{
    _Initialize();
}

Hd_USTC_CG_GL_RenderDelegate::Hd_USTC_CG_GL_RenderDelegate(const HdRenderSettingsMap& settingsMap)
    : HdRenderDelegate(settingsMap)
{
    _Initialize();
}

static void _RenderCallback(Hd_USTC_CG_GL_Renderer* renderer, HdRenderThread* renderThread)
{
    renderer->Clear();
    renderer->Render(renderThread);
}

std::mutex Hd_USTC_CG_GL_RenderDelegate::_mutexResourceRegistry;
std::atomic_int Hd_USTC_CG_GL_RenderDelegate::_counterResourceRegistry;
HdResourceRegistrySharedPtr Hd_USTC_CG_GL_RenderDelegate::_resourceRegistry;

void Hd_USTC_CG_GL_RenderDelegate::_Initialize()
{
...
Hd_USTC_CG_GL_RenderDelegate::~Hd_USTC_CG_GL_RenderDelegate()
{
    _resourceRegistry.reset();
    std::cout << "Destroying Tiny RenderDelegate" << std::endl;
}

const TfTokenVector& Hd_USTC_CG_GL_RenderDelegate::GetSupportedRprimTypes() const
{
    return SUPPORTED_RPRIM_TYPES;
}

const TfTokenVector& Hd_USTC_CG_GL_RenderDelegate::GetSupportedSprimTypes() const
{
    return SUPPORTED_SPRIM_TYPES;
}

const TfTokenVector& Hd_USTC_CG_GL_RenderDelegate::GetSupportedBprimTypes() const
{
    return SUPPORTED_BPRIM_TYPES;
}

HdResourceRegistrySharedPtr Hd_USTC_CG_GL_RenderDelegate::GetResourceRegistry() const
{
    return _resourceRegistry;
}

void Hd_USTC_CG_GL_RenderDelegate::CommitResources(HdChangeTracker* tracker)
{
}

HdRenderPassSharedPtr Hd_USTC_CG_GL_RenderDelegate::CreateRenderPass(
    HdRenderIndex* index,
    const HdRprimCollection& collection)
{
    std::cout << "Create RenderPass with Collection=" << collection.GetName() << std::endl;

    return std::make_shared<Hd_USTC_CG_GL_RenderPass>(
        index, collection, &_renderThread, _renderer.get(), &_sceneVersion);
}

HdRprim* Hd_USTC_CG_GL_RenderDelegate::CreateRprim(const TfToken& typeId, const SdfPath& rprimId)
{
    std::cout << "Create Tiny Rprim type=" << typeId.GetText() << " id=" << rprimId << std::endl;

    if (typeId == HdPrimTypeTokens->mesh) {
        return new Hd_USTC_CG_GL_Mesh(rprimId);
    }
    TF_CODING_ERROR("Unknown Rprim type=%s id=%s", typeId.GetText(), rprimId.GetText());
    return nullptr;
}

void Hd_USTC_CG_GL_RenderDelegate::DestroyRprim(HdRprim* rPrim)
{
    logging("Destroy Tiny Rprim id=" + rPrim->GetId().GetString(), USTC_CG::Info);
    delete rPrim;
}

HdSprim* Hd_USTC_CG_GL_RenderDelegate::CreateSprim(const TfToken& typeId, const SdfPath& sprimId)
{
    if (typeId == HdPrimTypeTokens->camera) {
        return new Hd_USTC_CG_GL_Camera(sprimId);
    }
    else if (typeId == HdPrimTypeTokens->extComputation) {
        return new HdExtComputation(sprimId);
    }
    else if (typeId == HdPrimTypeTokens->simpleLight || typeId == HdPrimTypeTokens->sphereLight) {
        return new Hd_USTC_CG_GL_Light(sprimId, typeId);
    }
    else {
...
HdSprim* Hd_USTC_CG_GL_RenderDelegate::CreateFallbackSprim(const TfToken& typeId)
{
    ...
    if (typeId == HdPrimTypeTokens->camera) {
        ...
        return new Hd_USTC_CG_GL_Light(SdfPath::EmptyPath(), typeId);
    }
    else {
        TF_CODING_ERROR("Unknown Sprim Type %s", typeId.GetText());
    }
...
void Hd_USTC_CG_GL_RenderDelegate::DestroySprim(HdSprim* sPrim)
{
    logging(sPrim->GetId().GetAsString() + " destroyed", USTC_CG::Info);
    delete sPrim;
}

HdBprim* Hd_USTC_CG_GL_RenderDelegate::CreateBprim(const TfToken& typeId, const SdfPath& bprimId)
{
    if (typeId == HdPrimTypeTokens->renderBuffer) {
        ...
        return new Hd_USTC_CG_GL_RenderBuffer(bprimId);
    }
    TF_CODING_ERROR("Unknown Bprim Type %s", typeId.GetText());
    return nullptr;
}

HdBprim* Hd_USTC_CG_GL_RenderDelegate::CreateFallbackBprim(const TfToken& typeId)
{
    if (typeId == HdPrimTypeTokens->renderBuffer) {
        return new Hd_USTC_CG_GL_RenderBuffer(SdfPath::EmptyPath());
    }
    TF_CODING_ERROR("Unknown Bprim Type %s", typeId.GetText());
    return nullptr;
}

void Hd_USTC_CG_GL_RenderDelegate::DestroyBprim(HdBprim* bPrim)
{
...
HdInstancer* Hd_USTC_CG_GL_RenderDelegate::CreateInstancer(
...
HdRenderParam* Hd_USTC_CG_GL_RenderDelegate::GetRenderParam() const
{
