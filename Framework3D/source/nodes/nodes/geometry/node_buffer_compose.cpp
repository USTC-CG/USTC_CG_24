...
namespace USTC_CG::node_buffer_compose {
...
#define NodeDeclare(type, suffix, size)                                              \
    static void node_declare_compose_buffer##size##suffix(NodeDeclarationBuilder& b) \
    {                                                                                \
        b.add_output<decl::type##size##Buffer>("Buffer");                            \
        for (int i = 0; i < size; ++i) {                                             \
            b.add_input<decl::type##1Buffer>(socket_name(i).c_str());                \
        }                                                                            \
    }
...
#define NodeExec(type, base_type, suffix, s)                                            \
    static void node_exec_compose_buffer##s##suffix(ExeParams params)                   \
    {                                                                                   \
        pxr::VtArray<type> Buffer;                                                      \
        pxr::VtArray<base_type> val[s];                                                 \
        for (int i = 0; i < s; ++i) {                                                   \
            val[i] = params.get_input<pxr::VtArray<base_type>>(socket_name(i).c_str()); \
        }                                                                               \
        size_t max_size = 0;                                                            \
        for (int i = 0; i < s; ++i) {                                                   \
...
#define NodeRegister(size, suffix)                                                     \
    ...
    strcpy_s(ntype_buffer##size##suffix.id_name, "geom_buffer_compose" #size #suffix); \
                                                                                       \
    geo_node_type_base(&ntype_buffer##size##suffix);                                   \
    ntype_buffer##size##suffix.node_execute = node_exec_compose_buffer##size##suffix;  \
    ntype_buffer##size##suffix.declare = node_declare_compose_buffer##size##suffix;    \
