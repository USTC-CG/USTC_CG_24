...
void IISPH::compute_pressure()
{
    ...
    for (unsigned iter = 0; iter < max_iter_; iter++) {
        ...
        if (enable_debug_output) {
            std::cout << YellowHead() << "[iter " << iter
                      << " ] avg density error = " << avg_density_error << ColorTail() << std::endl;
        }
...
void IISPH::predict_advection()
{
    ...
    for (auto& p : ps_.particles()) {
        ...
        for (auto& q : p->neighbors()) {
            ...
            //          = density_i - dt * density_i \sum mass_j / density_j (v_j - v_i) \cdot grad
            //          W_ij \approx density + dt *  \sum mass_j (v_i - v_j) \cdot grad W_ij
    ...
    for (auto& p : ps_.particles()) {
        ...
        for (auto& q : p->neighbors()) {
            Vector3d grad_W_ji =
                grad_W(q->x() - p->x(), ps_.h());  // Note: W_{ji} instead of W_{ij}
