...
using namespace pxr;
static void node_declare_add_distant_light(NodeDeclarationBuilder& b)
{
    b.add_input<decl::Float>("R").default_val(1).min(0).max(10);
    b.add_input<decl::Float>("G").default_val(1).min(0).max(10);
    b.add_input<decl::Float>("B").default_val(1).min(0).max(10);
...
static void node_exec_add_distant_light(ExeParams params)
{
    auto rotate_x = params.get_input<float>("Rotate X");
    auto rotate_y = params.get_input<float>("Rotate Y");
    auto rotate_z = params.get_input<float>("Rotate Z");
    ...
    auto angle = params.get_input<float>("Angle");
    pxr::UsdLuxDistantLight distant_light =
        pxr::UsdLuxDistantLight::Define(global_stage, lightPath);
    distant_light.CreateColorAttr().Set(VtValue(GfVec3f(r, g, b)));
    distant_light.CreateDiffuseAttr().Set(1.0f);
    distant_light.CreateAngleAttr().Set(angle);
}
...
static void node_register()
{
    ...
    ntype_add_distant_light.declare = node_declare_add_distant_light;
    ntype_add_distant_light.ALWAYS_REQUIRED = true;
