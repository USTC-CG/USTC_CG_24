...
#define ARRAY_LAST_ITEM(arr_start, arr_dtype, arr_len) \
    (arr_dtype *)((char *)(arr_start) +                \
                  (sizeof(*((arr_dtype *)NULL)) * (size_t)(arr_len - 1)))
...
#define ARRAY_DELETE(arr, index, delete_len, arr_len)                   \
    {                                                                   \
        static_assert(                                                  \
            (&arr[index] >= arr) && ((index) + delete_len <= arr_len)); \
        memmove(                                                        \
            &(arr)[index],                                              \
 ...
 * `if (index + 1 != arr_len) { arr[index] = arr[arr_len - 1]; }` (typical
 * reordering removal), with removing multiple items, overlap is detected to
 * avoid memcpy errors.
 */
#define ARRAY_DELETE_REORDER_LAST(arr, index, delete_len, arr_len)            \
    {                                                                         \
        ...
        if ((index) + (delete_len) != (arr_len)) {                            \
            if (((delete_len) == 1) ||                                        \
                ((delete_len) <= ((arr_len) - ((index) + (delete_len))))) {   \
                memcpy(                                                       \
                    &(arr)[index],                                            \
                ...
                memcpy(                                                       \
                    &(arr)[index],                                            \
                    &(arr)                                                    \
                        [(arr_len) - ((arr_len) - ((index) + (delete_len)))], \
                    ((arr_len) - ((index) + (delete_len))) * sizeof(*(arr))); \
            }                                                                 \
        }                                                                     \
    }                                                                         \
...
#define ARRAY_SIZE(arr)                                                       \
    ((sizeof(                                                                 \
          struct { int isnt_array : ((const void *)&(arr) == &(arr)[0]); }) * \
      0) +                                                                    \
...
/** Performs `offsetof(typeof(data), member) + sizeof((data)->member)` for
 * non-gcc compilers. */
#define OFFSETOF_STRUCT_AFTER(_struct, _member)       \
    ((size_t)(((const char *)&((_struct)->_member)) - \
              ((const char *)(_struct))) +            \
 ...
 * time. This isn't inclusive, the value of \a member isn't copied.
 */
#define MEMCPY_STRUCT_AFTER(struct_dst, struct_src, member)                    \
    {                                                                          \
        ...
        ((void)(struct_dst == struct_src),                                     \
         memcpy(                                                               \
             (char *)(struct_dst) + OFFSETOF_STRUCT_AFTER(struct_dst, member), \
             (const char *)(struct_src) +                                      \
                 OFFSETOF_STRUCT_AFTER(struct_dst, member),                    \
             sizeof(*(struct_dst)) -                                           \
                 OFFSETOF_STRUCT_AFTER(struct_dst, member)));                  \
    }                                                                          \
    ((void)0)
...
#define MEMSET_STRUCT_AFTER(struct_var, value, member)                        \
    {                                                                         \
        ...
        memset(                                                               \
            (char *)(struct_var) + OFFSETOF_STRUCT_AFTER(struct_var, member), \
            value,                                                            \
            sizeof(*(struct_var)) -                                           \
                OFFSETOF_STRUCT_AFTER(struct_var, member));                   \
    }                                                                         \
    ((void)0)
...
/**
 * WARNING: this doesn't warn when returning pointer types (because of the
 * placement of `*`). Use #UNUSED_FUNCTION_WITH_RETURN_TYPE instead in this
 * case.
 */
#if defined(__GNUC__) || defined(__clang__)
...
#define UNUSED_FUNCTION_WITH_RETURN_TYPE(rtype, x) \
    __attribute__((__unused__)) rtype UNUSED_##x
#else
 ...
 * \code{.py}
 * for i in range(16):
 *     args = [(chr(ord('a') + (c % 26)) + (chr(ord('0') + (c // 26)))) for c in
 * range(i + 1)] print("#define _VA_UNUSED_VARS_%d(%s) \\" % (i + 1, ",
 * ".join(args))) print("\t((void)(%s)%s)" % (args[0], ((", _VA_UNUSED_VARS_" +
 * str(i) + "(%s)") if i else "%s") %
 *              ", ".join((args[1:]))))
 * \endcode
 */
...
#define _VA_UNUSED_VARS_13(a0, b0, c0, d0, e0, f0, g0, h0, i0, j0, k0, l0, m0) \
    ((void)(a0),                                                               \
     _VA_UNUSED_VARS_12(b0, c0, d0, e0, f0, g0, h0, i0, j0, k0, l0, m0))
#define _VA_UNUSED_VARS_14(                                 \
    a0, b0, c0, d0, e0, f0, g0, h0, i0, j0, k0, l0, m0, n0) \
    ((void)(a0),                                            \
     _VA_UNUSED_VARS_13(b0, c0, d0, e0, f0, g0, h0, i0, j0, k0, l0, m0, n0))
#define _VA_UNUSED_VARS_15(                                     \
    a0, b0, c0, d0, e0, f0, g0, h0, i0, j0, k0, l0, m0, n0, o0) \
    ((void)(a0),                                                \
     _VA_UNUSED_VARS_14(                                        \
         b0, c0, d0, e0, f0, g0, h0, i0, j0, k0, l0, m0, n0, o0))
#define _VA_UNUSED_VARS_16(                                         \
    a0, b0, c0, d0, e0, f0, g0, h0, i0, j0, k0, l0, m0, n0, o0, p0) \
    ((void)(a0),                                                    \
     _VA_UNUSED_VARS_15(                                            \
         b0, c0, d0, e0, f0, g0, h0, i0, j0, k0, l0, m0, n0, o0, p0))
...
constexpr size_t ARRAY_SIZE(const T (&arg)[N]) noexcept
{
    (void)arg;
    return N;
}
